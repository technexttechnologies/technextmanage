...
import { updateErpPurchase } from '@/app/erp/integrations/aronium/actions';

export default function EditPurchaseModal({ purchase, onClose }: { purchase: any, onClose: () => void }) {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    await updateErpPurchase(purchase.id, {
      totalAmount,
      status
    });
    setIsSaving(false);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ marginBottom: '16px', color: 'var(--text-primary)', fontSize: '1.25rem' }}>Edit Purchase: {purchase.orderNumber}</h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div>
            <label style={{ display: 'block', marginBottom: '4px', color: 'var(--text-secondary)' }}>Status</label>
            <select
              value={status}
