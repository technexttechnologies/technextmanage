...
import { UploadCloud, Loader2, Save, Trash2, Plus, Edit3 } from "lucide-react";
...
export default function QuotationPdfUploader({ requestId }: { requestId: string }) {
  ...
  const handleSaveAndUpload = async () => {
    ...
    try {
      ...
      alert("PDF and AI Data uploaded successfully!");
      setParsedData(null);
      setFile(null);
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div style={{ marginTop: '16px', padding: '16px', background: 'var(--surface-background)', border: '1px dashed var(--brand-primary)', borderRadius: 'var(--radius-md)', textAlign: 'center' }}>
      ...
      />
      <label htmlFor="pdfUploadQuote" style={{ cursor: isUploading ? 'not-allowed' : 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
        <UploadCloud size={24} color="var(--brand-primary)" />
        <span style={{ fontWeight: 600, color: 'var(--brand-primary)' }}>
          Upload Official PDF from Aronium
        </span>
      </label>
    </div>
  );
