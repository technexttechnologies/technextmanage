      ...
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>
            {isAdmin ? "Manage and generate quotations requested by employees." : "Request new quotations for your customers."}
          </p>
        </div>
        <Link href="/quotation-requests/new" className="btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div className={styles.card}>
        {requests.length === 0 ? (
          <div className={styles.emptyState}>
            <FileSignature size={48} style={{ opacity: 0.2, margin: '0 auto 16px' }} />
            <h3>No Quotation Requests Found</h3>
            <p>Click "New Request" to create your first quotation request.</p>
          </div>
        ) : (
                ...
                {requests.map((req) => (
                  <tr key={req.id}>
                    ...
                    <td>
                      ...
                      <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{req.customer.company}</div>
                    </td>
                    <td>
                      <div style={{ fontWeight: 500 }}>{req.serviceName}</div>
                      <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                        {req.budget ? `Budget: ${req.budget}` : 'No budget specified'}
                      </div>
                    </td>
                    <td>
                      <span className={styles[`priority_${req.priority}`]}>
                        {req.priority}
                      </span>
                    </td>
                    <td>
